...
class DefaultHandler:
    ...
    def handle(self, data, feature):
        ...
        for k, v in col_dict.items():
            if not v:
                continue
            result_dict[k] = v(data, self.preprocessing_result[self.feature],
...
class ZScalingHandler(DefaultHandler):
    def handle(self, data, feature):
        ...
        feature_preprocessing_result = self.preprocessing_result[self.feature]
        result_dict = {}
        ...
        result_dict[standartization_column] = col_dict[standartization_column](
            data, self.preprocessing_result)
        result_dict[max_feature_id_key], max_value =\
